...
> **Verdict: our `src/chaos/` diagnostics recover the KNOWN answers of a benchmark whose classes
> are analytically settled — so they are trusted before we point them at the coupled SFC
> substrate.** This is the macro-dynamics analogue of the chaos suite's logistic λ=ln2 check, and
> it is **Steve Keen's own model**, so it doubles as the concrete debt-dynamics artifact.
...
|---|---|---|---|
| 0 | nesting | max\|Δ(ω,λ,d)\| = **0.0** | Keen(κ=id,r=δ=0,d₀=0) ≡ Goodwin, byte-exact |
| 1 | conservation | worst \|ΔH\| = **4.4e-14** | Lotka–Volterra invariant H is constant |
| 2 | eigenvalues (`jacobian`/`eigs`) | \|μ\| = **1.000000**, Ω = **0.3602** | centre; Ω = √(A·C) = 0.3603; +1 real d-eig contracts |
...
- **A2 — structurally non-hyperbolic equilibrium.** Goodwin's equilibrium is a *centre*: the
  (ω,λ) Jacobian has zero trace **for any** linear-Phillips config, so `|μ|=1` exactly (marginal,
  ∀ parameters). Our `jacobian`/`eigs` return it, and the conserved `H` is the reason — the same
  conservation-⇒-non-hyperbolic story as the chaos core (CYB-2/4), one dimension down.
