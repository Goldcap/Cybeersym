...
> **Research-status note.** The author’s formal academic background is outside mathematics, and this investigation has been substantially assisted by generative AI. AI was used to explore analytical approaches, locate relevant concepts and literature, implement numerical instruments, and test successive hypotheses. Its output has not been treated as authoritative. Structural statements identified below as proved follow directly from the stated map or from explicit computation; dynamical classifications identified as numerical remain provisional and require independent expert scrutiny.
...
[**Figure 1.** Bifurcation diagram obtained from downward and upward parameter sweeps, showing finite-amplitude onset and the coexistence interval.]
...
[**Figure 2.** Maximum modulus of the transverse equilibrium spectrum versus \(\beta\), plotted with the numerically chaotic interval. The spectral radius remains below approximately \(0.92\) in the principal continuation calculation.]
...
The map and numerical instruments are available in the public MIT-licensed repository `Goldcap/Cybeersym`, principally under `src/chaos/`. The implementation is deterministic and includes self-tests for the Lyapunov, linearization, and two-dimensional border-collision instruments.
